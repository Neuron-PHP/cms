...
<div class="container-fluid">
	<div class="d-flex justify-content-between align-items-center mb-4">
		<h2>Donations</h2>
		<form method="GET" action="<?= route_path('admin_donations') ?>" class="d-flex align-items-center gap-2">
	...
	<div class="card">
		<div class="card-body">
			...
			<?php else: ?>
								...
								<tr>
									...
									<td>
										<?php if( !empty( $d['donor_name'] ) || !empty( $d['donor_email'] ) ): ?>
											<?= htmlspecialchars( trim( ( $d['donor_name'] ?? '' ) . ( !empty( $d['donor_email'] ) ? ' <' . $d['donor_email'] . '>' : '' ) ) ) ?>
										<?php else: ?>
									...
									<td>
										<a href="<?= route_path('admin_donation_show', ['id' => $d['id']]) ?>" class="btn btn-sm btn-outline-primary" title="View">
											<i class="bi bi-eye"></i>
										</a>
										<form action="<?= route_path('admin_donation_delete', ['id' => $d['id']]) ?>" method="POST" class="d-inline" onsubmit="return confirm('Delete this donation record?');">
						...
						<ul class="pagination pagination-sm mb-0">
							...
							?>
								<li class="page-item <?= ( $p === ( $page ?? 1 ) ) ? 'active' : '' ?>">
									<a class="page-link" href="<?= route_path('admin_donations') ?>?page=<?= $p . $filterQs ?>"><?= $p ?></a>
								</li>
							<?php endfor; ?>
						</ul>
					</nav>
				...
				<p class="text-muted small mt-2 mb-0"><?= (int) ( $total ?? 0 ) ?> total donation(s)</p>
			<?php endif; ?>
		</div>
	</div>
</div>
