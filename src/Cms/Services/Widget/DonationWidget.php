...
/**
 * Donation form widget / shortcode.
 *
 * Renders a configurable donation form powered entirely by the `donations`
 * config section, so a single shortcode drives every donation form:
 *
 *   [donation]               -> default form
 *   [donation form="capital"] -> the "capital" campaign
 *   [donation form="general" title="Support Us" button="Give"]
 *
 * The form posts to /donations/checkout, which creates a hosted Stripe Checkout
 * session and redirects the donor. A CSRF token is fetched client-side from
 * /donations/token so the markup stays valid when the page is cached.
 *
 * @package Neuron\Cms\Services\Widget
 */
class DonationWidget implements IWidget
{
	/**
	...
	public function __construct( DonationService $donationService, ?SessionManager $sessionManager = null )
	{
	...
	{
		return 'Render a configurable donation form processed via hosted Stripe Checkout.';
	}
	...
	{
		return [
			'form'   => 'Form key from the donations config (default: configured default_form)',
	...
	{
		$key = (string) ( $attrs['form'] ?? $this->_donationService->getDefaultFormKey() );

		$config = $this->_donationService->getFormConfig( $key );

		if( $config === null )
		{
			return "<!-- donation widget: unknown form '" . $this->esc( $key ) . "' -->";
		}

		$fields = $config['fields'] ?? [];
		$title  = $attrs['title'] ?? ( $config['label'] ?? 'Make a Donation' );
		$button = $attrs['button'] ?? ( $config['button'] ?? 'Donate' );

		$idSuffix = preg_replace( '/[^a-z0-9_]/i', '_', $key );

		[ $successMessage, $errorMessage ] = $this->consumeFlash( $key );

		$html = '<div class="donation-form-widget mb-4">';

		if( $title !== '' )
		{
			$html .= '<h3 class="donation-form-title mb-3">' . $this->esc( (string) $title ) . '</h3>';
		}
		...
		$html .= '<form class="donation-form" method="POST" action="/donations/checkout" data-donation-form>';
	...
	{
		$symbol  = self::CURRENCY_SYMBOLS[ $this->_donationService->getCurrency() ] ?? '';
		$presets = $this->_donationService->presetAmounts( $key );

		$html  = '<fieldset class="mb-3 donation-amounts">';
		...
		foreach( $presets as $amount )
		{
			...
			$id    = 'donation_' . $idSuffix . '_amount_' . preg_replace( '/[^a-z0-9_]/i', '_', $value );
		...
		if( $this->_donationService->allowsCustomAmount( $key ) )
		{
			$customRadioId = 'donation_' . $idSuffix . '_amount_custom';
			$customInputId = 'donation_' . $idSuffix . '_custom_amount';
			...
			$html .= '<div class="input-group mt-2 donation-custom-amount">';
			...
			$html .= '<input type="number" class="form-control" id="' . $this->esc( $customInputId ) . '" name="custom_amount" min="' . $this->esc( $this->formatAmountValue( $this->_donationService->minimumAmount( $key ) ) ) . '" step="0.01" placeholder="Enter amount">';
			$html .= '</div>';
		}
	...
	{
		$frequencies = $this->_donationService->allowedFrequencies( $key );
		...
		$html  = '<fieldset class="mb-3 donation-frequencies">';
		...
		foreach( $frequencies as $frequency )
		{
			...
			$id    = 'donation_' . $idSuffix . '_freq_' . preg_replace( '/[^a-z0-9_]/i', '_', $frequency );
	...
	/**
	 * Render a single configured donor field as a Bootstrap control.
	 *
	 * @param array $field
	 * @param string $idSuffix
	 * @return string
	...
	private function renderField( array $field, string $idSuffix ): string
	{
		...
		$id       = 'donation_' . $idSuffix . '_' . preg_replace( '/[^a-z0-9_]/i', '_', $name );
	 ...
	 * it into every donation form, keeping cached markup valid.
	 *
	 * @return string
	 */
...
<script>
(function() {
	if( window.__donationFormTokenInit ) { return; }
	window.__donationFormTokenInit = true;
	document.addEventListener('DOMContentLoaded', function() {
		fetch('/donations/token', { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' })
			.then(function(r) { return r.json(); })
			.then(function(data) {
				if( !data || !data.token ) { return; }
				document.querySelectorAll('form[data-donation-form] input[name="csrf_token"]').forEach(function(input) {
...
HTML;
	...
	private function consumeFlash( string $key ): array
	{
		...
		$success = $session->getFlash( 'donation.' . $key . '.success' );
		$error   = $session->getFlash( 'donation.' . $key . '.error' );
